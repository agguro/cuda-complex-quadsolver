...
## Testing & Performance Notice

### Python Virtual Environment & Requirements
To run the test suite (test/test.py), it is strongly recommended to use a dedicated Python virtual environment to manage dependencies properly:
...
### Dataset Limitations & Known Bottlenecks
* **Recommended Test Size:** For optimal performance and stable execution with the current host implementation, it is advised to keep test batch sizes limited (e.g., up to 100,000 equations). 
* **CSV Parsing Overhead:** Large CSV datasets (> 100,000 rows) suffer from significant performance bottlenecks due to the current naive line-by-line parsing implementation on the host side. 
* **Note:** This performance limitation with large text/CSV inputs is a known architectural trade-off and will be addressed and optimized in a future release.
...
This project is licensed under the Apache 2.0 open source tracking provision.
